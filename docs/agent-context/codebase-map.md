...
```
No tests exist for `app/app.py` (the callback/entry-point module — including the new upload-stage/confirm-mapping callbacks), `app/src/dimension_reduction_functions.py`, `app/src/session_manager.py`, `app/src/callbacks.py`, or `app/pages/*` — confirmed via `Glob test/src/*.py` (7 files) vs. 11 non-empty modules in `app/src/` + `app/app.py` + `app/pages/`.
...
### 4. Redis session persistence (BROKEN as currently committed)
...
- **Bug**: `app/app.py:26-31` has the import of these functions commented out, but three callbacks (`update_redis_keys` line 248, `load_session_data` line 273, `save_session_data_to_redis` line 319) call `list_keys`, `load_from_redis`, `save_to_redis` unconditionally. These will raise `NameError` at call time — the Redis save/load/list UI (sidebar "Load-Save Session" section, `app/pages/home.py:139-156`) is currently non-functional. Untouched by the mapping refactor: Redis round-trips the whole `session` dict as one opaque blob, so the `meta_data` shape (unchanged) is transparent to it — no extra work needed there if/when this gets fixed.
...
- **Python deps**: `app/requirements.txt` (UTF-16LE with CRLF — see Fragile section). Pinned versions include `dash==2.14.2`, `Flask==2.2.5`, `Flask-Caching==2.1.0` (imported nowhere directly found in `app/src` or `app/app.py` besides `cache_initialize.py`'s key-builder function, which itself isn't wired into a `Cache` object anywhere I found — **cache key logic appears unused/dead**, see Fragile section), `pacmap==0.7.2`, `scikit-learn==1.4.1.post1`, `gunicorn==20.1.0`, `redis==3.5.3`.
...
- **Error handling is inconsistent**: `app/app.py` callbacks mix bare `try/except Exception as e: print(...)` (e.g. `update_redis_keys`, `save_session_data_to_redis`) with no exception handling at all elsewhere (e.g. `update_map`, `plot_data`). `DataPlotter.initialize_data` (`app/src/data_manager.py`) catches broadly and re-raises as `ValueError`. The upload/mapping flow is the one place with structured, per-field error/warning reporting (`ValidationIssue`/`ValidationResult` in `app/src/data_mapping.py`) — everywhere else still relies on raw `print()` or nothing.
- **`print()` used throughout for debugging/status** instead of `logging` (no `logging` module used anywhere in `app/`).
- Docstrings are present and fairly thorough in `app/src/data_process.py`, `app/src/data_mapping.py`, `app/src/dimension_reduction_functions.py`/`compositional_data_functions.py` (numpy-style), but largely absent in `app/app.py` and `app/src/data_manager.py` (only a couple of inline `#` comments).
- Commented-out dead code blocks are generally left in place rather than deleted elsewhere in the repo (e.g. `app/src/data_manager.py`'s legacy `SessionManager.package_session_data` static method) — **the regex-based column-classification code this refactor replaced was fully deleted, not commented out**, since the user explicitly ruled out maintaining a backward-compat path; don't take the general "leave dead code in place" pattern as covering that removal.

## Fragile / Risky Areas
- **Redis save/load/list callbacks are broken** as committed (`app/app.py` — see Workflow 4). This is the single biggest functional gap found, unaffected by this refactor.
...
- **Flask-Caching is a declared dependency and `app/src/cache_initialize.py` builds a cache key function (`make_custom_cache_key_dimensionReduction`) and a df-hash function**, but no `Cache(app, ...)` object or `@cache.memoize`/`@cache.cached` decorator was found anywhere in `app/app.py` or `app/src/`. PCA/PaCMAP re-runs on every "Apply" click regardless of the hash. Untouched by this refactor.
- **No test coverage** for `app/app.py` (all callback wiring/business logic glue, including the new upload-stage/confirm-mapping/group-color callbacks), `app/src/dimension_reduction_functions.py` (PCA/PaCMAP pipeline), `app/src/session_manager.py` (Redis I/O), `app/src/callbacks.py`, or `app/pages/*` (layout). Covered: `data_manager`, `data_process`, `data_model`, `data_mapping`, `compositional_data_functions`, `plotting`, `cache_initialize`.
- **`clr_transform` mutates its input in place** (`X[X == 0.0] = np.nan` at `app/src/compositional_data_functions.py:38`) — could cause surprising aliasing bugs if callers reuse the source dataframe/array afterward. Untouched by this refactor.
